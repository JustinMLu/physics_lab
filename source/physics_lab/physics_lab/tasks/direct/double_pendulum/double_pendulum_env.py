    ...
    def _get_observations(self) -> dict:
        obs = torch.cat(
            (
                ...
                normalize_angle(self.joint_pos[:, self._pole_dof_idx[0]]).unsqueeze(dim=1),
                self.joint_vel[:, self._pole_dof_idx[0]].unsqueeze(dim=1),
                normalize_angle(self.joint_pos[:, self._pendulum_dof_idx[0]]).unsqueeze(dim=1),
...
def compute_rewards(
    ...
    total_reward = (
        rew_alive + rew_termination + rew_pole_pos + rew_cart_vel + rew_pole_vel + rew_pendulum_pos + rew_pendulum_vel
    )
    return total_reward